...
int nEpoch = 1;                      // max training epochs
...
void Forward(XTensor inputs[], XTensor &output, FNNModel &model, FNNNet &net, double & totalTime);
...
void ForwardTest(XTensor inputs[], XTensor &output, FNNModel &model, FNNNet &net, double& totalTime);
/* 
...
{	
	...
	XPRINT1(0, stderr, "\n\n ~~~~~~~~  softmax total running time is %f seconds\n\n", totalTime);
    ...
    for(epoch = 0; epoch < nEpoch; epoch++){
        ...
        while(ngramNum > 0){
            ...
            if(!autoDiff){
                ...
                /* forward computation */
                ForwardTest(inputs, output, model, net, totalTime);

				//Forward(inputs, output, model, net, totalTime);

                /* backward computation to obtain gradients */
				/* problem no backward, no update */
            ...
            if (step % 100 == 0) {
				...
				XPRINT1(0, stderr, " ~~~~~~~~  softmax total running time is %.5f seconds\n\n", totalTime);
            }
        }
...
{
	/*
		modify to get Prob check the X_INT
	*/
    ...
    /* problem ������������Σ� result��wprobs��Ľ��һ�������� Ч�ʿ������ܵ� */
...
void Forward(XTensor inputs[], XTensor &output, FNNModel &model, FNNNet &net, double & totalTime)
{
       ...
       TODO: we add bias term here */
    {
		...
		totalTime += GetClockSec() - start;
		
		/* y = softmax(s) */
		/* problem
		   here think that the matrix's value has been 0.
		   so scale
		*/
		/*s = ScaleAndShift(s, 100);
		
		s = ConvertDataType(s, X_INT);
		y = ConvertDataType(y, X_INT);
		
		_LinerSoftmax(&s, &y, 1);
		
		y = ConvertDataType(y, DEFAULT_DTYPE);*/
    }
}
...
void ForwardTest(XTensor inputs[], XTensor &output, FNNModel &model, FNNNet &net, double& totalTime)
{
	...
	TODO: we add bias term here */
	{
		...
		*/
		...
		_LinerSoftmax(&s, &y, 1);
		//_FloatTestSoftmax(&s, &y, 1);
		
		start = GetClockSec();

		s = ConvertDataType(s, DEFAULT_DTYPE);
		y = ConvertDataType(y, DEFAULT_DTYPE);

		y = ScaleAndShift(y, 0.0001);
